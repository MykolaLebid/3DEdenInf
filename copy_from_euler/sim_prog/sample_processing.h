...
class ProbeMutProcessor {
  ...
  friend float dist_mean_pass_to_cell(const ProbeMutProcessor &,
	...
	friend float dist_branch_score(const ProbeMutProcessor &,  // dist of Kuhner and
																 const ProbeMutProcessor &, int seed); // Felsenstein (1994)
	...
	friend float dist_robinson_and_foulds(const ProbeMutProcessor &,
																				const ProbeMutProcessor &, int seed);	// friend float

	friend float nodal_distance (const ProbeMutProcessor &,
												       const ProbeMutProcessor &);

  friend float splitted_nodal_distance (const ProbeMutProcessor &,
																				const ProbeMutProcessor &);

	friend float dist_splitted_nodal (const ProbeMutProcessor &,
																		const ProbeMutProcessor &);

	friend float dist_nodal (const ProbeMutProcessor &,
													 const ProbeMutProcessor &);
	// dist_type = 10
	friend float t_dist_deg_vs_dist_nodal(const ProbeMutProcessor &,
													              const ProbeMutProcessor &);

	// dist_type = 11
	friend float t_dist_deg (const ProbeMutProcessor & Probe_mut_1,
									         const ProbeMutProcessor & Probe_mut_2);
	// dist_type = 12
	friend float t_dist_control (const ProbeMutProcessor & Probe_mut_1,
															const ProbeMutProcessor & Probe_mut_2);
protected:
  /////////////////////////////////////////////////////////////////////////////
