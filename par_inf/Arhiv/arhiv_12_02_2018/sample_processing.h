...
class TreeMutAnalyzer {
  ...
  TreeMutAnalyzer(const std::vector <Cell> &,
									const std::vector <Genotype *> & genotype_vector);  // TODO
  //Destructor:
  ~TreeMutAnalyzer();
  ...
  std::vector <Cell> raw_probe_cells;
...
public:
	//Constructor
	BalkVAFAnalyzer();
	BalkVAFAnalyzer(std::vector <Cell> & cell_vec,/*vector<Genotype*> & class_genotypes,*/
	...
	std::vector <float> get_cumulative_vaf_vec() const {return cumulative_vaf_vec;};
	std::vector <unsigned int> get_num_mutations_vec() const {return num_mutations_vec;};
	void set_num_mutations_vec(const std::vector<unsigned int> & a){ num_mutations_vec = a;};
	void CleanAll();
...
public:
	//Constructor
	CellMutAnalyzer();
	CellMutAnalyzer(std::vector <Cell> & cell_vec,/*vector<Genotype*> & class_genotypes,*/
								                             /*unsigned int total_number_of_SNV,*/
									std::vector<Genotype*> & cell_genotypes);
	...
	std::vector <Cell> GetCellVec() const { return cell_vec;};
	void CleanAll(){
		genotype_vec.clear();
		cell_vec.clear();
	};
	...
	void SetCellVec(std::vector <Cell> & _cell_vec){
