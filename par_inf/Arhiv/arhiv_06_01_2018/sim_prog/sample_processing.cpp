...
string TreeMutAnalyzer::FormCellString(int mut_vec_index)  const {
...
string TreeMutAnalyzer::GetCurrentMutationString(int mut_vec_index) const {
...
string TreeMutAnalyzer::get_str_newick_mut_tree_format() const {
	string str=";"/*="0d;"*/;
	str = "):1.0" + str;
...
vector <vector<int>> TreeMutAnalyzer::GetSPLMatrix() const {
...
void TreeMutAnalyzer::ConstructorInit() {
...
TreeMutAnalyzer::TreeMutAnalyzer(const vector <Cell> & _probe_cells,
...
void GetCumulativeDists (const vector <unsigned int> vec_1,
												const vector <unsigned int> vec_2,
												vector <float> & comulative_vec_1,
												vector <float> & comulative_vec_2) {
  vector <unsigned int> vec1(vec_1);
  vector <unsigned int> vec2(vec_2);
...
float KuiperDist(const vector <unsigned int> vec_1,
								 const vector <unsigned int> vec_2) {
	vector <float> comulative_vec_1, comulative_vec_2;
	GetCumulativeDists (vec_1, vec_2, comulative_vec_1, comulative_vec_2);
	int size_of_vecs = comulative_vec_1.size();
	float max_up = 0;
	float max_down = 0;
...
float KolmogorovSmirnovDist(const vector <unsigned int> vec_1,
								            const vector <unsigned int> vec_2){
	vector <float> comulative_vec_1, comulative_vec_2;
	GetCumulativeDists (vec_1, vec_2, comulative_vec_1, comulative_vec_2);
	int size_of_vecs = comulative_vec_1.size();
...
float CramerMisesDist(const vector <unsigned int> vec_1,
											const vector <unsigned int> vec_2) {
	vector <float> comulative_vec_1, comulative_vec_2;
	GetCumulativeDists (vec_1, vec_2, comulative_vec_1, comulative_vec_2);
	int size_of_vecs = comulative_vec_1.size();
...
vector <unsigned int> TreeMutAnalyzer::get_degree_mut_vec() const {
...
vector <unsigned int> TreeMutAnalyzer::get_depth_mut_vec() const {
...
float t_dist_depth_cum_kolmogorov_smirnov(const TreeMutAnalyzer & Probe_mut_1,
															                  const TreeMutAnalyzer & Probe_mut_2){
  vector <unsigned int> depth_mut_vec_pr_1 = Probe_mut_1.get_depth_mut_vec();
  vector <unsigned int> depth_mut_vec_pr_2 = Probe_mut_2.get_depth_mut_vec();
...
float t_dist_deg_cum_kolmogorov_smirnov(const TreeMutAnalyzer & Probe_mut_1,
															                  const TreeMutAnalyzer & Probe_mut_2){
  vector <unsigned int> deg_mut_vec_pr_1 = Probe_mut_1.get_degree_mut_vec();
  vector <unsigned int> deg_mut_vec_pr_2 = Probe_mut_2.get_degree_mut_vec();
...
float diff_mean_vectors(vector <unsigned int> vec_1, vector <unsigned int> vec_2){
...
void GetComparisonResultsFromFile(const AllSimParameters & pars,
															    vector<ComparisonResults> & comparison_results){
	...
	if (comparison_file.is_open()) {
		do {
			...
			comparison_file >> current.evolution.driver_adv >>
		        ...
		        current.evolution.driver_mutation_rate >> current.balk_dist >>
		        current.tree_dist >> current.dist;
			/*if (!s_in.eof())*/
...
vector <ComparisonResults> GetAvarageVec(const AllSimParameters & pars,
															           const vector<ComparisonResults> & comparison_results){
  vector <ComparisonResults> averaging_vec;
//
...
void MakeAverageFile(const AllSimParameters & pars){
	      ...
	      vector <ComparisonResults> average_vec = GetAvarageVec(pars,comparison_results);
...
void ProcessCellMutProbePiece(const AllSimParameters & pars,
		...
		ifstream file_tree_mut_vec_stream;
...
void GetTreeCellVectorFromBalkVector(const BasicSimParameters & pars,
														         const vector <Cell> & balk_cell_vector,
																		 vector <Cell> & tree_cell_vector) {
...
BalkVAFAnalyzer::BalkVAFAnalyzer(vector <Cell> & cell_vec,
												 vector<Genotype*> & cell_genotypes){
  ...
  //goes through set of gen indexes and gather mutations in set of muts
	...
	for(unsigned int cell_gene_index: cell_gene_index_set){
	  vector <unsigned int> sequence = cell_genotypes.at(cell_gene_index)->sequence;
...
vector <unsigned int> CellMutAnalyzer::GetCellMutFr(){
	//vector <unsigned int> CellFr
	vector <unsigned int> cell_gen_mut_num_vec = GetMutCellFr();
  //std::cout<<"size"<< cell_gen_mut_num_vec.size()<<"\n";
 	vector <unsigned int> num_mutations_vec;
...
vector <unsigned int> CellMutAnalyzer::GetMutCellFr(){
  //goes through set of gen indexes and gather mutations in set of muts
	std::set<unsigned int> gen_mut_set;

	for(Genotype current_genotype: genotype_vec){
	  vector <unsigned int> sequence = current_genotype.sequence;
  ...
  std::vector <unsigned int> cell_gen_mut_num_vec;
	...
	for (unsigned int current_gen_mut: gen_mut_set){
		...
		for (Cell current_cell: cell_vec) {
			...
			vector <unsigned int> genotype_mut_seq;
			for (Genotype current_genotype : genotype_vec)
...
float ZhaoMichorDist(CellMutAnalyzer cell_mut_probe_1,
	...
	vector <unsigned int> cell_mut_fr_1 = cell_mut_probe_1.GetCellMutFr();
	vector <unsigned int> cell_mut_fr_2 = cell_mut_probe_2.GetCellMutFr();
...
std::istream & operator>>(std::istream & s_in,
	...
	float current;
	vector <unsigned int> current_num_mut_vec;
	do {
		s_in >> current;
...
CellMutAnalyzer::CellMutAnalyzer(vector <Cell> & cell_vector, vector<Genotype*> & cell_genotypes) {
  std::set<unsigned int> cell_gene_index_set;
  //inserts cell index of gen to set of gen indexes
...
std::ostream & operator<<(std::ostream & output,
		...
		vector <Genotype> genotype_vec = cell_mut_analyzer.GetGenotypeVec();
		vector <Cell> cell_vec = cell_mut_analyzer.GetCellVec();
...
std::istream & operator>>(std::istream & s_in,
  ...
  unsigned int current_cell_gen;
	...
	for (unsigned int i = 0 ; i < num_of_genotypes; i++) {
			...
			vector <unsigned int> mut_index_genotype_vec;
