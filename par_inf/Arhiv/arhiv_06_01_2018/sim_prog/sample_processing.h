...
struct MutationHub {
  ...
  vector <unsigned int> label_vec; // we can have several labels adjusted to the hub
...
class TreeMutAnalyzer {
  ...
  TreeMutAnalyzer();
  TreeMutAnalyzer(const vector <Cell> &);
  TreeMutAnalyzer(const vector <Cell> &, unsigned int threshold_numb_cells);
  TreeMutAnalyzer(const vector <Cell> &,
  ...
  string get_str_newick_mut_tree_format() const;
  /////////////////////////////////////////////////////////////////////////////
  private:
  // Member variables
  ...
  vector <MutationHub> mut_hub_vector;
  ...
  void ChangeDepth(vector <MutationNode> & ,
													 unsigned int);
	void SetIndexFatherHubVec();
	void AdjustLabels2HubVecUsingMut();
  void AdjustLabels2HubVecUsingHub();
  vector <vector<int>> GetSPLMatrix() const;// Splitted path lengths
...
protected:
  ...
  void SaveNewickTreeFormat(char *);
	...
	string FormCellString(int num_cells) const; // for tree Newick format
  /////////////////////////////////////////////////////////////////////////////
...
public:
    //Constructor
    BalkVAFAnalyzer();
    BalkVAFAnalyzer(vector <Cell> & cell_vec,/*vector<Genotype*> & class_genotypes,*/
								/*unsigned int total_number_of_SNV,*/
							  vector<Genotype*> & cell_genotypes);
		void PrintToFileCumulativeDist(std::ostream & output);
		unsigned int get_cell_num() const {return num_of_cells;};
    unsigned int get_mut_num() const {return num_of_muts;};
	  vector <float> get_cumulative_vaf_vec() const {return cumulative_vaf_vec;};
	  vector <unsigned int> get_num_mutations_vec() const {return num_mutations_vec;};
	  void set_num_mutations_vec(const vector<unsigned int> & a){ num_mutations_vec = a;};
	  void CleanAll();

	  friend float KolmogorovSmirnovDist(const BalkVAFAnalyzer & balk_vaf_1,
																			 const BalkVAFAnalyzer & balk_vaf_2);
		friend float KuiperDist(const BalkVAFAnalyzer & balk_vaf_1,
														const BalkVAFAnalyzer & balk_vaf_2);
		friend float CramerMisesDist(const BalkVAFAnalyzer & balk_vaf_1,
														 const BalkVAFAnalyzer & balk_vaf_2);
    friend std::ostream & operator<<(std::ostream & output, const
...
private:
    ...
    vector <float> cumulative_vaf_vec;
...
public:
    //Constructor
    CellMutAnalyzer();
    CellMutAnalyzer(vector <Cell> & cell_vec,/*vector<Genotype*> & class_genotypes,*/
								                             /*unsigned int total_number_of_SNV,*/
							      vector<Genotype*> & cell_genotypes);
    ...
    vector <Genotype> GetGenotypeVec() const {return genotype_vec;};
    vector <Cell> GetCellVec() const { return cell_vec;};
    void CleanAll(){
			genotype_vec.clear();
			cell_vec.clear();
		};
    void SetGenotypeVec (vector <Genotype> & _genotype_vec){
    	genotype_vec = std::move(_genotype_vec);
		};
    void SetCellVec(vector <Cell> & _cell_vec){
		 cell_vec = std::move(_cell_vec);
    };
		void MutationFilter(const unsigned int left_border,
												const unsigned int right_border);
    vector <unsigned int> GetMutCellFr();
		vector <unsigned int> GetCellMutFr();
