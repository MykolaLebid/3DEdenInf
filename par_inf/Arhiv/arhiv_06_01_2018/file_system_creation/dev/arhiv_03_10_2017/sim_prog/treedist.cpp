...
double    compute_distances(pattern_elm ***, long, long);
...
void    output_double_distance(double diffd, long tree1, long tree2,
                long trees_in_1, long trees_in_2);
double    symetric_diff(group_type **tree1, group_type **tree2,
...
double    tree_diff(group_type **tree1, group_type **tree2, double *lengths1,
...
double compute_distances(pattern_elm ***pattern_array, long trees_in_1,
                long trees_in_2)
{
  ...
  switch (tree_pairing) {
    ...
    case ADJACENT_PAIRS:
      ...
      for (tree_index = 0 ; tree_index < end_tree ; tree_index += 2) {
        ...
        result_dist= tree_diff (treeA, treeB, length1, length2, patternsz1, patternsz2,
...
void getoptions()
{
  ...
  /* Initial settings */
  dtype          = BSD; //Mykola SYMMETRIC
  tree_pairing   = ALL_IN_1_AND_2; // NO_PAIRING, ADJACENT_PAIRS,CORR_IN_1_AND_2
																	 // ALL_IN_FIRST, ALL_IN_1_AND_2
  output_scheme  = VERBOSE;
  ibmpc          = IBMCRT;
  ansi           = ANSICRT;
  didreroot      = false;
...
void GetPhylipOptions(distance_type dist_type)
{
...
//    boolean done;
    ...
    /* Initial settings */
    dtype          = dist_type; //Mykola SYMMETRIC
    tree_pairing   = ADJACENT_PAIRS; // ALL_IN_FIRST, NO_PAIRING, ADJACENT_PAIRS,CORR_IN_1_AND_2
																		// , ALL_IN_1_AND_2
    output_scheme  = VERBOSE;
    ibmpc          = IBMCRT;
    ansi           = ANSICRT;
...
float phylip_dist(std::string & Probe_mut_1,
  ...
  node * p;
	...
	char name_buffer_array [L_tmpnam] = INTREE,
			 name_buffer_array_output_file [L_tmpnam] = OUTFILE;
// create unique prefix name for additional files///////////////////////////////
	std::mt19937 gen(time(NULL) + seed);
	std::uniform_int_distribution<int> rnd(1,200000000);
	//int i=0;
	// Check existences of file
	...
	do {
		...
		ss_out.str("");
		int rand = rnd(gen);
		ss_in << name_buffer_array << rand;
		ss_out << name_buffer_array_output_file << rand;

		//std::cout<<name_buffer_array<<std::endl;
		//std::cout<<name_buffer_array_output_file<<std::endl;
	} while( (File_Exists(ss_in.str().c_str())==true) &&
					 (File_Exists(ss_out.str().c_str())==true)   );

	strcpy(name_buffer_array, ss_in.str().c_str());
	strcpy(name_buffer_array_output_file, ss_out.str().c_str());

////////////////////////////////////////////////////////////////////////////////
//  #if defined __linux
    //mkstemp(name_buffer_array);
		//mkstemp(name_buffer_array_output_file);
		//tmpnam (name_buffer_array);
//  #else
		//mkstemp(name_buffer_array);
		//mkstemp(name_buffer_array_output_file);
		//tmpnam (name_buffer_array);
		//tmpnam (name_buffer_array_output_file);
//  #endif
	//std::cout << name_buffer_array << std::endl;
  //printf ("Tempname #1:  %s\n",name_buffer_array);
  std::ofstream tree1(name_buffer_array);
  //std::ofstream output_file_ofstream(name_buffer_array_output_file);

  tree1 << Probe_mut_1 << std::endl << Probe_mut_2 <<std::endl;
 // output_file_ofstream << std::endl;
//	output_file_ofstream << std::endl;
...
//     their values. */
  ...
  if (dist_type==0) GetPhylipOptions(SYMMETRIC);
		else GetPhylipOptions(BSD);

  openfile(&intree, name_buffer_array, "input tree file", "rb",  intreename);
  openfile(&outfile, name_buffer_array_output_file, "output file", "w",  outfilename);

	//outfile = fopen("openfile.txt", "w");
	//outfilename = "openfile.txt";
	//
	//	char *tree_buf_1, *tree_buf_2;
	//	size_t len_buf_1, len_buf_2;
	//
//
//
...
//	intree2 = tmpfile ();
//
//	fputs ( Probe_mut_1.c_str(), intree);
//	fputs ( Probe_mut_2.c_str(), intree2);
//
//	rewind(intree);
//	rewind(intree2);
  ...
  if ( (tree_pairing == ALL_IN_1_AND_2)
    ...
    openfile(&intree2, INTREE2, "input tree file 2", "rb", intree2name);
  ...
  if ( (tree_pairing == ADJACENT_PAIRS)
    ...
    /* Here deal with the adjacent or all-in-first pairing
       difference computation */
			 distance = compute_distances (pattern_array, trees_in_1, 0);
 ...
 // FClose(outtree);
  ...
  if( remove(name_buffer_array) != 0 ||
		  remove(name_buffer_array_output_file) != 0)
			std::cout << "Error in deleting additional Phylo files" << std::endl;
 //   else
 //   std::cout << "File successfully deleted" << std::endl;


//  std::cout<<"dist_topo = "<< distance << "; sqrt(dist_topo) = " << sqrt(distance) << std::endl;
  return distance;
