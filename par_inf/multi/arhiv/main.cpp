...
void AddToLogFile(Parameters & pars, unsigned int iteration_num,
									unsigned int trials, TreeComparison & relative_error, TreeComparison & variance);
TreeComparison GetMeanParDist(Parameters & pars,
															std::vector<TreeComparison>& tree_comparison_vec);
TreeComparison GetSampleVarianceParDist(Parameters & pars,
															std::vector<TreeComparison>& tree_comparison_vec);
...
void MainLoop(Parameters & pars,
	...
	while ((pars.vars.max_dist > pars.algo.final_epsilon) &&
				 ...
				 (iteration_num < pars.algo.final_iteration)){//the begin of main loop
    ...
    float diff = tree_comparison_vec.size() -
		...
		iteration_num++;
		TreeComparison treeComparisonRelativeError = GetMeanParDist(pars, tree_comparison_vec);
		TreeComparison treeComparisonSampleVariance = GetSampleVarianceParDist(pars, tree_comparison_vec);
...
TreeComparison GetMeanParDist(Parameters & pars,
  ...
  // 1)
	...
	// 2)
	A.par_point.driver_adv = A.par_point.driver_adv/(1.0*tree_comparison_vec.size());
	A.par_point.mut_rate = A.par_point.mut_rate/(1.0*tree_comparison_vec.size());
	A.par_point.driver_mut_rate = A.par_point.driver_mut_rate/(1.0*tree_comparison_vec.size());

	// 3)
	A.par_point.driver_adv = (pars.search_space.driver_adv.true_val - A.par_point.driver_adv)/
	                          pars.search_space.driver_adv.true_val;
	A.par_point.mut_rate = (pars.search_space.mut_rate.true_val - A.par_point.mut_rate)/
													pars.search_space.mut_rate.true_val;
	A.par_point.driver_mut_rate = (pars.search_space.driver_mut_rate.true_val -
	                              A.par_point.driver_mut_rate)/
	                              pars.search_space.driver_mut_rate.true_val;
...
TreeComparison GetSampleVarianceParDist(Parameters & pars,
  ...
  float var_1=0, var_2=0, var_3=0;
	...
	A.par_point.driver_adv = var_1/(1.0 * tree_comparison_vec.size());
	A.par_point.mut_rate = var_2/(1.0 * tree_comparison_vec.size());;
	A.par_point.driver_mut_rate = var_3/(1.0 * tree_comparison_vec.size());;
...
void AddToLogFile(Parameters & pars,
									unsigned int iteration_num,
									unsigned int trials,
									TreeComparison & relative_error,
									TreeComparison & sample_var) {
	std::ofstream log_tree_dist_comparison;
	char name_file[256];
	sprintf(name_file,"%s/%s", pars.catalog_file_name.c_str(),
														 pars.log_file_name.c_str());
	log_tree_dist_comparison.open(name_file,std::ios_base::app);
	if (log_tree_dist_comparison.is_open()==false) cout<<"err..."<<endl;
	...
	log_tree_dist_comparison << iteration_num<<"\t"<<
	                            ...
	                            sample_var.par_point.driver_mut_rate<< endl;
