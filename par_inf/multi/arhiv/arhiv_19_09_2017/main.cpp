...
void InitPars(const int argc, char *argv[], Parameters & pars); // function in main
void InitLoop(Parameters & pars); // function in main
...
//	const std::vector<TreeComparison> & tree_comparison_vec, Parameters & pars);
...
//																							tree_comparison_vec);
//TreeComparison GetMeanParDist(Parameters & pars,
...
//		const std::vector<TreeComparison> & array_of_tree_comparisons,
//		const ublas::matrix<float, ublas::row_major> & epsilon_matrix,
//		const int i, const int j, const Parameters & pars);
////
//////////////////////////////////////////////////////////////////////////////////
//
//// next key function in InnerLoop()
//void SubInnerLoop(const std::vector<TreeComparison> & tree_comparison_vec,
//					        Parameters & pars, ublas::matrix<float, ublas::row_major> & Cholesky_matrix);
//////////////////////////////////////////////////////////////////////////////////
...
//ublas::vector<float> ReflectedCoords(Parameters & pars,
...
int main (int argc, char *argv[]){
  ...
  if (argc == expected_arg_num){
	...
	if (FileExists(full_par_file_name.c_str())){
		pars.vars.gen.seed(pars.conf_settings.basic_sim_parameters.seed);
		InitLoop(pars);
		std::vector<TreeComparison> tree_comparison_vec;
//		int par_pop_num = pars.conf_settings.inference_parameters.
//												algorithm_parameters.parameter_population_number;
		//GetTreeComparisonVecFromFile(pars, par_pop_num, tree_comparison_vec);
...
//  do {
//		if ((GetLeftBorderDriverAdvantage(pars)<=new_parameter_vector[0] &&
//			GetRigthBorderDriverAdvantage(pars)>=new_parameter_vector[0]) || is_in_dom_vec[0] ) {
...
//															   GetLeftBorderDriverAdvantage(pars),
//															   GetRigthBorderDriverAdvantage(pars));
//		}; // process driver_adv coord
//		if ((GetLeftBorderMutationRate(pars)<=new_parameter_vector[1] &&
//				GetRigthBorderMutationRate(pars)>=new_parameter_vector[1]) || is_in_dom_vec[1]) {
...
//															   GetLeftBorderMutationRate(pars),
//															   GetRigthBorderMutationRate(pars));
//		}; // process mut_rate coord
//		if ((GetLeftBorderDriverMutationRate(pars)<=new_parameter_vector[2] &&
//				GetRigthBorderDriverMutationRate(pars)>=new_parameter_vector[2]) || is_in_dom_vec[2]) {
...
//															   GetLeftBorderDriverMutationRate(pars),
//															   GetRigthBorderDriverMutationRate(pars));
...
//	for (int i=0; i < size_matrix; i++) new_parameter_vector[i] = n_d(pars.vars.gen);
...
//#endif
//
//	if (pars.conf_settings.inference_parameters.
...
//	// reflect coords of parameters
//	// until all of them will be in acceptable parameter range
//	ReflectedCoords(pars, new_parameter_vector);
//#if defined(DEBUG_REGIME)
...
//				const std::vector<TreeComparison> & array_of_tree_comparisons,
//				const ublas::matrix<float, ublas::row_major> & epsilon_matrix,
//				const int i, const int j, Parameters & pars) {
//#if defined(DEBUG_REGIME)
//  std::cout<<"In GetGaussianKernel="<<std::endl;
//#endif
////	int parameter_number;
//	std::vector <bool> is_par_vec;
//	if (pars.conf_settings.inference_parameters.log_scale_search_space.driver_advantage == 0) {
//		is_par_vec.push_back(false);
//	} else {
//		is_par_vec.push_back(true);
//	};
//	if (pars.conf_settings.inference_parameters.log_scale_search_space.mutation_rate == 0) {
//		is_par_vec.push_back(false);
//	} else {
//		is_par_vec.push_back(true);
//	};
//	if (pars.conf_settings.inference_parameters.log_scale_search_space.driver_mutation_rate == 0) {
//		is_par_vec.push_back(false);
//	} else {
//		is_par_vec.push_back(true);
//	};
...
//	int parameter_population_num_alpha = pars.conf_settings.inference_parameters.
...
//	parameter_vector = GetMultiNormalDistSample(parameter_vector, pars, Cholesky_matrix);
...
//												Parameters & pars,
//												std::vector<TreeComparison>& tree_comparison_vec){
//	int index_of_begin = pars.conf_settings.inference_parameters.
//											 algorithm_parameters.parameter_population_num_alpha;
...
//							 const ublas::matrix<float, ublas::row_major> & epsilon_matrix){
//	int matrix_size = GetSearchParNumber();
//	int parameter_population_number = pars.conf_settings.inference_parameters.
//														algorithm_parameters.parameter_population_number;
//	int parameter_population_num_alpha = pars.conf_settings.inference_parameters.
//														algorithm_parameters.parameter_population_num_alpha;
...
//  std::string file_name(GetFullComparisonFileName(pars));
...
//				SubInnerLoop(tree_comparison_vec, pars, L);
//			};
//	} while(GetNumLinesInComparisonFile(pars) <= diff);
//
//  std::vector<TreeComparison> tree_comparison_vec_plus;
//  GetTreeComparisonVecFromFile(pars, diff, tree_comparison_vec_plus);
...
//		                           parameter_population_num_alpha];
//	SetNewWeights2NewTreeComparisons(epsilon_matrix, pars, tree_comparison_vec);
//};
//
//ublas::matrix<float, ublas::row_major> GetTwiceWeightedEmpiricalCovariance(
//										const ublas::vector<float> & weighted_mean_vector,
...
//	// 3)
//	float true_driver_adv = pars.conf_settings.basic_sim_parameters.evolution.driver_adv;
//	A.par_point.driver_adv = (true_driver_adv - A.par_point.driver_adv) / true_driver_adv;
//	float ture_mut_rate = pars.conf_settings.basic_sim_parameters.evolution.mutation_rate;
//	A.par_point.mut_rate = (ture_mut_rate - A.par_point.mut_rate) / ture_mut_rate;
//	float true_dr_mut_rate = pars.conf_settings.basic_sim_parameters.evolution.driver_mutation_rate;
...
//					                  op);
//	if (pars.conf_settings.inference_parameters.log_scale_search_space.driver_advantage == 0) {
//		weighted_mean_vector[0] = pars.conf_settings.basic_sim_parameters.evolution.driver_adv;
//	};
//
//	if (pars.conf_settings.inference_parameters.log_scale_search_space.mutation_rate == 0) {
//		weighted_mean_vector[1] = pars.conf_settings.basic_sim_parameters.evolution.mutation_rate;
//	};
//
//	if (pars.conf_settings.inference_parameters.log_scale_search_space.driver_mutation_rate == 0) {
//		weighted_mean_vector[2] = pars.conf_settings.basic_sim_parameters.evolution.driver_mutation_rate;
//	};
//
...
//
//	float final_epsilon = pars.conf_settings.inference_parameters.
//													algorithm_parameters.final_epsilon;
//	float p_acc_min = pars.conf_settings.inference_parameters.
//													algorithm_parameters.p_acc_min;
//	float final_iteration = pars.conf_settings.inference_parameters.
//													algorithm_parameters.final_iteration;
//	int parameter_population_num_alpha = pars.conf_settings.inference_parameters.
//														algorithm_parameters.parameter_population_num_alpha;
//	while ((pars.vars.max_dist > final_epsilon) && (pars.vars.p_acc > p_acc_min) &&
...
//																			GetMeanParDist(pars, tree_comparison_vec);
//		TreeComparison treeComparisonSampleVariance =
//														GetSampleVarianceParDist(tree_comparison_vec);
//		AddToLogFile(pars, iteration_num, pars.vars.n_trials,
...
void InitLoop(Parameters & pars){
	...
	std::mt19937 gen(rd());
	std::uniform_real_distribution<float> driver_adv_gen(GetLeftBorderDriverAdvantage(pars),
																											 GetRigthBorderDriverAdvantage(pars));
	std::uniform_real_distribution<float> mut_rate_gen(GetLeftBorderMutationRate(pars),
																										 GetRigthBorderMutationRate(pars));
	std::uniform_real_distribution<float> driver_mut_rate_gen(GetLeftBorderDriverMutationRate(pars),
																														GetRigthBorderDriverMutationRate(pars));
	int parameter_population_number = pars.conf_settings.inference_parameters.
																			algorithm_parameters.parameter_population_number;
	do {//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	} while (GetNumLinesInComparisonFile(pars) <= parameter_population_number);
};

void InitPars(int argc, char *argv[], Parameters & pars) {
		pars.conf_settings.basic_sim_parameters.related_path_to_par_file = argv[1];
		pars.conf_settings.basic_sim_parameters.par_file_name = argv[2];
		GetAllParameters(pars.conf_settings.basic_sim_parameters,
										 pars.conf_settings.inference_parameters);
		pars.conf_settings.basic_sim_parameters.seed = atoi(argv[3]);
};
