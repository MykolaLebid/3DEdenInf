MAIN_CATALOGUE_NAME="all_results/FS_dist_comparison_fit_adv_0_1__mut_r_0_10__dr_mut_r_0_001_20052018"    	

SIMULATION_MODE='2' # 1 - for ABC inference; 2 - distance comparison; 3 - statistics 
NUM_OF_POINTS_IN_SEARCH_SPACE="100"
NUM_OF_REPETITIONS="1"
LEFT_SCALE="0.9"
RIGHT_SCALE="0.9"
MODE_OF_DIST="2" # 0 - we scale right coefficient to 1 and run comparison for one par
	         # that is appropriate  for fit ad parameter and driver probability parameter
                 # 1 - we scale right coefficient to 1 run comparison for all par
		 # 2 - we don't scale right coefficient to 1 and run comparison for one par; that is appropriate  for mutational rate parameter 
ITERATION_MODE_OF_DIST="1" # 0 - fit ad
			   # 1 - mut rate
			   # 2 - dr mut rate
...
# file system creation
  ...
  '1')
  	...
  	cd dev	 
   		./compiling.bat   
   		./run.bat $MAIN_CATALOGUE_NAME $SIMULATION_MODE $NUM_OF_POINTS_IN_SEARCH_SPACE $NUM_OF_REPETITIONS $LEFT_SCALE $RIGHT_SCALE $MODE_OF_DIST $ITERATION_MODE_OF_DIST
  	cd ..
  	cd ..	
  ;;
  '2')
