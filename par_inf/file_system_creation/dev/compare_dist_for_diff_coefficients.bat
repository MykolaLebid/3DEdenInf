  ...
  do
    ...
    do
      ...
      do
        for((i_4=0;i_4<=2;i_4++))
        do
          bsub -n 1 -W 240 -R "rusage[mem=1024]"  -R "rusage[scratch=0]" cancer_3d.exe ..\FS_test\driver_adv_$i_0\mutation_rate_$i_1\driver_mutation_rate_$i_2\seed_$i_3\tree_dist_type_$i_4\ init_conf_file.dat $i_0$i_1$i_2$i_3$i_4
        done
      done
    done
  done
