...
void StatisticsFS::Get(std::string & initial_config_file_name,
  ...
  std::string local_name  = ".." + GetSystemRelevantSlash() + initial_config_file_name;
	initial_config.GetFromFile(local_name);
	initial_config_f_n = initial_config_file_name;

	local_name = ".." + GetSystemRelevantSlash() + step_lengths_file_name;
	step_lengths.GetFromFile(local_name);
	step_lengths_f_n = step_lengths_file_name;

	local_name = ".." + GetSystemRelevantSlash() + step_numbers_file_name;
	step_numbers.GetFromFile(local_name);
	step_numbers_f_n = step_numbers_file_name;

	local_name = ".." + GetSystemRelevantSlash() + orders_file_name;
...
///////////////////////Create upper directory///////////////////////////////////
  std::string command_line ("mkdir ");
	command_line += GetSystemRelevantUpperRoot() + sim_directory_name;
	system(command_line.c_str());
////////////////////////////////////////////////////////////////////////////////
	CreateCatalogesAndFiles(0, sim_directory_name, initial_config);
...
std::string StatisticsFS::GetSystemRelevantUpperRoot(){
	std::string slash = GetSystemRelevantSlash();
	std::string two_points("..") ;
	std::string upper_cataloge(two_points + slash + two_points + slash);
	return upper_cataloge;
};
...
std::string StatisticsFS::GetSystemRelevantInit(){
...
void StatisticsFS::CreateCatalogesAndFiles(unsigned int tuple_index,
	...
	if (tuple_index < vec_parameter_tuples.size()) {
		...
		for (int i = 0; i < step_number; i++) {
			...
			if ((tuple_index + 1) == vec_parameter_tuples.size()) {
				std::string path_to_conf_file = GetSystemRelevantUpperRoot() + local_cataloge_name +
																				slash + initial_config_f_n;

				parameters.SetToFile(path_to_conf_file);
			};
			CreateCatalogesAndFiles(tuple_index + 1, local_cataloge_name, parameters);
		};
	};
};

void StatisticsFS::ModifyCurrentParameters(Parameters & parameters,
																					 int tuple_index,
																					 int current_step){
...
void StatisticsFS::BashOpenLoops(std::string & bash_file_name) {
...
void StatisticsFS::BashCloseLoops(std::string & bash_file_name){
		std::ofstream bash_file;
	  bash_file.open(bash_file_name, std::ios_base::app);
		unsigned int number_of_parameters = vec_parameter_tuples.size();
		std::string space("  ");
...
void StatisticsFS::CreateBashForIndependenSim(std::string & bash_file_name,
    ...
    locale_name_of_setting_file += sim_derectory_name + GetSystemRelevantSlash();
		for (unsigned int i = 0; i < number_of_parameters; i++)
		...
		bash_file << locale_space <<"bsub -n "<< proc_num <<" -W "
							...
							<< GetSystemRelevantInit() << GetSystemRelevantUpperRoot()
							<< "sim_prog" << GetSystemRelevantSlash() << prog_file_name <<  " "
							<< locale_name_of_setting_file
							<< initial_config_f_n <<" "
							<< locale_name_of_setting_file <<"etalon" << std::endl;
  ...
  unsigned int number_of_parameters = vec_parameter_tuples.size();
	...
	for_each (vec_of_dirs.begin(), vec_of_dirs.end(), [&](std::string str) {
								...
								bash_file << locale_space<< "mkdir -p "
													<< sim_derectory_name << GetSystemRelevantSlash()
