...
//
void GetEtalonProbeParAndInitValues(Parameters & pars) {
...
void GetInferencePar(Parameters & pars) {
  ...
  // end*
  pars.evolution.stop_time_cell_num = pt.get<int>
  ...
  switch( argc ){
    ...
    case expected_arg_num_for_simulation: { // 2
      //if (IsSimulationForInference(argv[1])) {
        ...
        pars.evolution.driver_adv = atof(argv[2]);
        pars.evolution.mutation_rate = atof(argv[3]);
        pars.evolution.driver_mutation_rate = atof(argv[4]);
...
void DoesEtalonProbPart(Parameters & pars) {
    if (FileExists(pars.par_file_name.c_str())){
      GetEtalonProbeParAndInitValues(pars); // EtalonProbe
      if (run(pars)) {
