...
bool run(const bool is_etalon_sim,
	...
	//init data
	resetSimData(pars.evolution.driver_adv, pars.evolution.driver_mutation_rate, sim_data);
	...
	switch (pars.evolution.simulation_regime) {
		case 0:{// well mixed
			while ((mainProgWellMixed(pars.evolution, sim_data)==1) &&
				...
				resetSimData(pars.evolution.driver_adv, pars.evolution.driver_mutation_rate,
							sim_data);
				s++;
