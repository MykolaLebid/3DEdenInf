...
float phylip_dist(std::string & Probe_mut_1,
								  std::string & Probe_mut_2, int dist_type) {
  ...
  std::cout<<"before probe" <<std::endl;
	...
	//FILE *intree=NULL, *intree2=NULL, *outfile=NULL;
  node * p;


	std::ofstream tree(INTREE);
		tree <<  Probe_mut_1 <<std::endl;
  	tree << Probe_mut_2 <<std::endl;
  	tree.close();
	char name_buffer_array [L_tmpnam];
  ...
  if ( (tree_pairing == ADJACENT_PAIRS)
    ...
    /* Here deal with the adjacent or all-in-first pairing
       difference computation */
			 double d=compute_distances (pattern_array, trees_in_1, 0);
